...
\begin{example*}[Private Dating]
    ...
    If $x = 1$, Alice places it as $dog$ on top of $cat$ \(i.e. closer to the already filled dog card\), and if $y = 1$, Bob places it as $dog$ on top of $cat$ \(i.e. closer to the already filled doing card\) as well. Otherwise, they flip them. If $x = y = 1$, then the $dog$'s will be adjacent. If $x \neq y$, the order will be $dog,dog,cat,dog,cat$ (the $cat$'s are not adjacent), regardless of which of Alice or Bob produced $x = 0$ (or both!).
\end{example*}
